...
int parseOption(int argc, char * const argv[], struct Args* p_args)
{
	...
	}else if(args->ts_id > 0){
		if(args->ts_id != args->channel_info.tsid){
			ret = CH_RETURN_NOTFOUND;
		}
	}
...
#ifdef STD_B25
static void init_STD_B25(ARIB_STD_B25 ** const  pB25, B_CAS_CARD ** const  pB25cas, void *devst)
{
	int r;
...
struct OutputBuffer* create_TSParser(unsigned  bufSize, struct OutputBuffer* const  pOutput, const unsigned  mode, void *devst)
{
	...
	/* initialize decoder */
	if(mode & 0x1) {
		init_STD_B25(&( prv->b25 ), &( prv->b25cas ), devst);
	}else{
		prv->b25 = NULL;
		prv->b25cas = NULL;
	}
